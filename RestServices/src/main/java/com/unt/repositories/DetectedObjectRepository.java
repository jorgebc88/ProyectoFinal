...
public interface DetectedObjectRepository extends JpaRepository<DetectedObject,Long> {
    ...
    @Query("SELECT do FROM DETECTED_OBJECTS WHERE do.Date = :date")
    List<DetectedObject> findByDate(@Param("date")Calendar date);

    @Query("SELECT do FROM DETECTED_OBJECTS WHERE :day = EXTRACT(DAY FROM do.Date)")
    List<DetectedObject> findByDay(@Param("day")Calendar day);

    @Query("SELECT do FROM DETECTED_OBJECTS WHERE :month = EXTRACT(DAY FROM do.Date)")
    List<DetectedObject> findByMonth(@Param("month")Calendar month);

    @Query("SELECT do FROM DETECTED_OBJECTS WHERE :year = EXTRACT(MONTH FROM do.Date)")
    List<DetectedObject> findbyYear(@Param("year")Calendar Year);

    @Query("SELECT do FROM DETECTED_OBJECTS WHERE do.Date BETWEEN :startDate AND :endDate")
